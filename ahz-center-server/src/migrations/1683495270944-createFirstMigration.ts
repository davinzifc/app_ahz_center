...
export class createFirstMigration1683495270944 implements MigrationInterface {
    name = 'createFirstMigration1683495270944'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`user\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`created_by\` bigint NULL DEFAULT NULL, \`updated_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`updated_by\` bigint NULL DEFAULT NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`user_id\` bigint NOT NULL AUTO_INCREMENT, \`first_name\` text NOT NULL, \`last_name\` text NULL DEFAULT NULL, \`email\` text NOT NULL, \`password\` text NOT NULL, \`is_application\` tinyint NOT NULL DEFAULT 0, INDEX \`IDX_758b8ce7c18b9d347461b30228\` (\`user_id\`), PRIMARY KEY (\`user_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user_by_role\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`created_by\` bigint NULL DEFAULT NULL, \`updated_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`updated_by\` bigint NULL DEFAULT NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`user_id\` bigint NOT NULL, \`role_id\` bigint NOT NULL, INDEX \`IDX_9c4fa227bbc07f95fcfd13c00d\` (\`user_id\`, \`role_id\`), PRIMARY KEY (\`user_id\`, \`role_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`restore_password_token\` (\`token\` varchar(40) NOT NULL, \`restore_user_id\` bigint NOT NULL, \`expiry_date\` date NOT NULL, PRIMARY KEY (\`token\`)) ENGINE=InnoDB`);
